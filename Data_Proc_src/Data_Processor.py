...
reportfile = os.path.join("..",reportfolder,f"Insurance_report_{timenow}.xlsx")
...
confirmed_new = confirmed_long.groupby(['Country/Region', 'Date']).agg(confirmed=('confirmed', 'sum')).reset_index()
...
deaths_new = deaths_long.groupby(['Country/Region', 'Date']).agg(deaths=('deaths', 'sum')).reset_index()
...
reportfile = os.path.join("..",reportfolder,f"Covid_India_report_{timenow}.csv")
